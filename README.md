...
A C++ and Python framework for controlling Trossen robotic arms using VR controllers via WebSocket communication. The SDK operates in server mode, listening for incoming connections from VR rigs and translating controller poses and button presses into robot commands.
...
5. [Running C++ Scripts](#running-c-scripts)
6. [Running Python Scripts](#running-python-scripts)
...
- [Boost.Asio](https://www.boost.org/) (async I/O, typically bundled with websocketpp)
- pthread (threading support)
- [libtrossen_arm](https://github.com/TrossenRobotics/libtrossen_arm) (Trossen robot driver)
...
# Install libtrossen_arm (follow instructions from Trossen Robotics)
# Typically available via pip:
...
- **C++ executables**: `teleop`, `teleop2` (example scripts in `scripts/cpp/`)
...
The C++ example scripts are located in `scripts/cpp/` and compiled as executables in the `build/` directory.

### test_script1 (Event-Driven Mode)
...
### test_script2 (Manual Polling Mode)
...
#### test_script1.py (Event-Driven Mode)

```bash
python3 test_script1.py
```
...
#### test_script2.py (Manual Polling Mode)
